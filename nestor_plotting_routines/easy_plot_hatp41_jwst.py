...
# Extract samples:
pic=pickle.load(open('./OUTPUT/hp41_wfc3_cc_jwst_two-hem.pic','rb'), \
                encoding='latin1')
samples=pic[:,:-1]
...
if not os.path.exists('model_wavelengths_jwst.npy'):
...
def gen_model(cube, wavelengths):
    ...
    P,T, H2Oarr, CH4arr,COarr,CO2arr,NH3arr,Naarr,Karr,TiOarr,VOarr,C2H2arr,HCNarr,H2Sarr,FeHarr,H2arr,Hearr,Harr, earr, Hmarr,qc,r_eff,f_r = foo2[-1]
...
if os.path.exists('model_wavelengths_jwst.npy'):

    model_samples = np.load('model_samples_jwst.npy')
    depth1_samples = np.load('model_depth1_samples_jwst.npy')
    depth2_samples = np.load('model_depth2_samples_jwst.npy')
    wavelengths_model = np.load('model_wavelengths_jwst.npy')
    ...
    np.save('model_wavelengths_jwst.npy', wavelengths_model)
    np.save('model_depth1_samples_jwst.npy', depth1_samples)
    np.save('model_depth2_samples_jwst.npy', depth2_samples)
    np.save('model_samples_jwst.npy', model_samples)
...
plt.tight_layout()
plt.savefig('retrieval_jwst.pdf')
...
plt.tight_layout()
plt.ylim(4900,5600)
plt.savefig('jwst_limb_constraint.pdf')
