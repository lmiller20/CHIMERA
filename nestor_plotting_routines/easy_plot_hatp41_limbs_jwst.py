...
# Extract samples:
pic=pickle.load(open('./OUTPUT/hp41_wfc3_cc_jwst_two-limbs.pic','rb'), \
                encoding='latin1')
samples=pic[:,:-1]
...
if plot_corner:
    ...
    plt.savefig('plot_comb_jwst.png')

# Now plot models and spectra. First load cross sections:
if not os.path.exists('model_wavelengths_jwst-limbs.npy'):
...
"""
wlgrid1, cl_d1, cl_derr1, hl_d1, hl_derr1 = np.loadtxt('limb_data_order1.dat',unpack=True)
wlgrid2, cl_d2, cl_derr2, hl_d2, hl_derr2 = np.loadtxt('limb_data_order2.dat',unpack=True)
wlgrid = np.append(wlgrid1, wlgrid2) 
...
if os.path.exists('model_wavelengths_jwst-limbs.npy'):

    model_samples = np.load('model_samples_jwst-limbs.npy')
    depth1_samples = np.load('model_depth1_samples_jwst-limbs.npy')
    depth2_samples = np.load('model_depth2_samples_jwst-limbs.npy')
    wavelengths_model = np.load('model_wavelengths_jwst-limbs.npy')
    ...
    wavelengths_model = np.linspace(0.36, 3.0, 300) 
    ...
    np.save('model_wavelengths_jwst-limbs.npy', wavelengths_model)
    np.save('model_depth1_samples_jwst-limbs.npy', depth1_samples)
    np.save('model_depth2_samples_jwst-limbs.npy', depth2_samples)
    np.save('model_samples_jwst-limbs.npy', model_samples)
...
plt.tight_layout()
plt.savefig('retrieval_jwst-limbs.pdf')

# Plot JWST constraint on possible limbs:
plt.figure(figsize=(6,3))
...
plt.tight_layout()
plt.ylim(4900,5600)
plt.savefig('jwst_limb_constraint-limbs.pdf')
