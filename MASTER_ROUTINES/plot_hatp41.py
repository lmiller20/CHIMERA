...
pic=pickle.load(open('./OUTPUT/hp41_wfc3_cc_two-hem.pic','rb'), encoding='latin1')
samples=pic[:,:-1]
lnprob=pic[:,-1]
print(samples.shape)
sys.exit()
outname='hp41_wfc3_cc_two-hem'


# corner plot
titles=np.array(['T$_{irr}$', '[M/H]', 'log(C/O)', 'log(K$_{zz}$)', 'f$_{sed}$' ,'log(P$_{b}$)','log(VMR$_{cld,b}$)','$\\times$R$_p$','$log(K$_{IR}$)$', '$log(\gamma_{1})$'])
priorlow=np.array([ 1000, -2,  -2,  5.0,  0.5, -6.0, -15., 0.5, -3., -3.])
priorhigh=np.array([2500, 3.0, 0.3, 11,   6.0,  1.5,  -2,  1.5, 0., 0.])
Npars=len(titles)
...
for i in range(len(draws)):
    ...
    Tirr, logMet, logCtoO, logKzz, fsed ,logPbase,logCldVMR,xRp, logKir, logg1 = samples[draws[i],:]
