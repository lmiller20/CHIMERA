...
# Extract samples:
pic=pickle.load(open('/user/lmiller/CHIMERA_results/OUTPUT_Lauren_test/hp41_wfc3_cc_jwst_two-limbs-cov.pic','rb'), \
                encoding='latin1')
samples=pic[:,:-1]
...
if plot_corner:
    ...
    plt.savefig('plot_comb_jwst-cov.png')

# Now plot models and spectra. First load cross sections:
if not os.path.exists('model_wavelengths_jwst-limbs-cov.npy'):
...
if os.path.exists('model_wavelengths_jwst-limbs-cov.npy'):

    model_samples = np.load('model_samples_jwst-limbs-cov.npy')
    depth1_samples = np.load('model_depth1_samples_jwst-limbs-cov.npy')
    depth2_samples = np.load('model_depth2_samples_jwst-limbs-cov.npy')
    wavelengths_model = np.load('model_wavelengths_jwst-limbs-cov.npy')
    ...
    np.save('model_wavelengths_jwst-limbs-cov.npy', wavelengths_model)
    np.save('model_depth1_samples_jwst-limbs-cov.npy', depth1_samples)
    np.save('model_depth2_samples_jwst-limbs-cov.npy', depth2_samples)
    np.save('model_samples_jwst-limbs-cov.npy', model_samples)
...
plt.tight_layout()
plt.savefig('retrieval_jwst-limbs-cov.pdf')
...
plt.tight_layout()
plt.ylim(4900,5600)
plt.savefig('jwst_limb_constraint-limbs-cov.pdf')
