...
#import run
pic=pickle.load(open('OUTPUT/TOI-199_transmission_jwst_free_NEWSOSS_TAKE2.pic','rb'), encoding='latin1')
samples=pic[:,:-1]
lnprob=pic[:,-1]

outname='pmn_transmission_jwst_freeNEWSOSS-TAKE2'
...
ss = samples[:,:5]
titles=np.array(['T$_{iso}$', 'log(H2O)','log(CO)','log(CO2)','log(CH4)'])
priorlow=np.array([  100, -12,  -12,-12 ,-12])
priorhigh=np.array([ 800, 0,    0.,  0,   0])
Npars=len(titles)
ext=np.zeros([2,Npars])
ext=ext.T
ext[:,0]=priorlow
...
#'''
corner.corner(ss,labels=titles, bins=25,plot_datapoints='False',quantiles=[.16,0.5,.84],show_titles='True',plot_contours='True',extents=ext,levels=(1.-np.exp(-(1)**2/2.),1.-np.exp(-(2)**2/2.),1.-np.exp(-(3)**2/2.)))
...
fill_between(1E4/wno[::-1],y_low_2sig[::-1]*100,y_high_2sig[::-1]*100,facecolor='cornflowerblue',alpha=0.5,edgecolor='None',zorder=0)
fill_between(1E4/wno[::-1],y_low_1sig[::-1]*100,y_high_1sig[::-1]*100,facecolor='cornflowerblue',alpha=0.75,edgecolor='None',zorder=0)
plot(1E4/wno, y_median*100,'cornflowerblue',zorder=3)
