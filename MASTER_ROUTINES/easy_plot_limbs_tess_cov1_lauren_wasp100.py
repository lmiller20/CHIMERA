...
# Extract samples:
pic=pickle.load(open('/user/lmiller/CHIMERA_results/OUTPUT_Lauren_new/wasp100_tess_cc_two-limbs-cov_new.pic','rb'), \
                encoding='latin1')

#pic=pickle.load(open('/user/lmiller/CHIMERA_results/OUTPUT_Lauren_test/wasp100_tess_cc_two-limbs-cov.pic','rb'), \
                #encoding='latin1')
...
if plot_corner:
    ...
    plt.savefig('plot_comb_tess-cov_wasp100_new.png')
    #plt.savefig('plot_comb_tess-cov_wasp100_test.png')

# Now plot models and spectra. First load cross sections:
if not os.path.exists('model_wavelengths_tess-limbs-cov_wasp100_new.npy'):
#if not os.path.exists('model_wavelengths_tess-limbs-cov_wasp100_test.npy'):
...
def gen_model(cube, wavelengths):
    ...
    Rp = 1.330 #Stassun et al. 2017, Exoplanet Archive #0.930#*x[4]# Planet radius in Jupiter Radii--this will be forced to be 10 bar radius--arbitrary (scaling to this is free par)
    Rstar = 1.77457 #TICv8, Exoplanet Archive #0.598   #Stellar Radius in Solar Radii
    M = 1.26 #Stassun et al. 2017, Exoplanet Archive #1.78    #Mass in Jupiter Masses
...
"""
#wlgrid1, cl_d1, cl_derr1, hl_d1, hl_derr1,cov1 = np.loadtxt('/home/lmiller/TSRC/final_run/WASP-100b.dat',unpack=True)
wlgrid1, hl_d1, hl_derr1, cl_d1, cl_derr1,cov1 = np.loadtxt('/home/lmiller/TSRC/final_run/WASP-100b_new.dat',unpack=True)
#wlgrid1, hl_d1, hl_derr1, cl_d1, cl_derr1,cov1 = np.loadtxt('/home/lmiller/TSRC/final_run/WASP-100b.dat',unpack=True)
...
if os.path.exists('model_wavelengths_tess-limbs-cov_wasp100_new.npy'):
#if os.path.exists('model_wavelengths_tess-limbs-cov_wasp100_test.npy'):

    model_samples = np.load('model_samples_tess-limbs-cov_wasp100_new.npy')
    depth1_samples = np.load('model_depth1_samples_tess-limbs-cov_wasp100_new.npy')
    depth2_samples = np.load('model_depth2_samples_tess-limbs-cov_wasp100_new.npy')
    wavelengths_model = np.load('model_wavelengths_tess-limbs-cov_wasp100_new.npy')
    
#     model_samples = np.load('model_samples_tess-limbs-cov_wasp100_test.npy')
#     depth1_samples = np.load('model_depth1_samples_tess-limbs-cov_wasp100_test.npy')
#     depth2_samples = np.load('model_depth2_samples_tess-limbs-cov_wasp100_test.npy')
#     wavelengths_model = np.load('model_wavelengths_tess-limbs-cov_wasp100_test.npy')
    ...
    np.save('model_wavelengths_tess-limbs-cov_wasp100_new.npy', wavelengths_model)
    np.save('model_depth1_samples_tess-limbs-cov_wasp100_new.npy', depth1_samples)
    np.save('model_depth2_samples_tess-limbs-cov_wasp100_new.npy', depth2_samples)
    np.save('model_samples_tess-limbs-cov_wasp100_new.npy', model_samples)
    
#     np.save('model_wavelengths_tess-limbs-cov_wasp100_test.npy', wavelengths_model)
#     np.save('model_depth1_samples_tess-limbs-cov_wasp100_test.npy', depth1_samples)
#     np.save('model_depth2_samples_tess-limbs-cov_wasp100_test.npy', depth2_samples)
#     np.save('model_samples_tess-limbs-cov_wasp100_test.npy', model_samples)
...
plt.tight_layout()
plt.savefig('retrieval_tess-limbs-cov_wasp100_new.pdf')
#plt.savefig('retrieval_tess-limbs-cov_wasp100_test.pdf')
...
#plt.ylim(5500,7300)
plt.tight_layout()
plt.savefig('tess_limb_constraint-limbs-cov_wasp100_new.pdf')
#plt.savefig('tess_limb_constraint-limbs-cov_wasp100_test.pdf')
