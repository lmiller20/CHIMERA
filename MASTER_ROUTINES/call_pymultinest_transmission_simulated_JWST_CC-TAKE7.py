...
outpath="./OUTPUT/pmn_transmission_jwst-FINAL-TAKE7-SOSS_cc"
...
def loglike(cube, ndim, nparams):
    ...
    Tirr, logMet, logCtoO, logKzz, fsed ,logPbase,logCldVMR,xRp,logPQCarbon, logPQNitrogen=cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9]
...
def prior(cube,ndim,nparams):
    ...
    cube[5] = 9*cube[5]-6.0  #logPbase: -6.0 - 3 (1 ubar - 1000 bar)
    cube[6] = 10*cube[6]-10  #logCldVMR: -10 - 0
    cube[7] = 1*cube[7]+0.5  #xRp: 0.5 - 1.5 (multiplicative factor to "fiducial" 10 bar radius)
    cube[8] = 8.*cube[8]-5.0  # quench pressure for Carbon and Oxygen compounds; form 10**(-5) to 10**(3) bars (i.e., 1 microbar down to 1000 bars).
    cube[9] = 8.*cube[9]-5.0  # quench pressure for Nitrogen compounds; form 10**(-5) to 10**(3) bars (i.e., 1 microbar down to 1000 bars).

#####loading in data##########
wlgrid, y_meas, err=np.loadtxt('chimera_toi199-2-sc-soss.dat').T
